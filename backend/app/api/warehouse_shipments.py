...
from db.classes import WarehouseShipment
...
router = APIRouter()
svc = ApiService(db.warehouse_shipments_table, warehouse_shipment_schema)
table_class = WarehouseShipment
base_path = "/warehouse_shipments/"
...
def get_shipped_items(item_id: int):
    return svc.get_items_by_id_template(db.get_table_by_name("warehouse_shipped_items"), "warehouseShipmentId", item_id)
